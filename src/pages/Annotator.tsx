...
const Annotator = ({
    ...
    // --- FIX 1: Add Refs to track the last successfully synchronized props ---
    const lastInitialAnnotationsRef = useRef(initial_Annotations);
    const lastInitialPathsRef = useRef(initial_Paths);
    // ------------------------------------------------------------------------
    ...
    useEffect(() => {
        const isAnnotationContentEqual = JSON.stringify(initial_Annotations) === JSON.stringify(lastInitialAnnotationsRef.current);
        const isPathContentEqual = JSON.stringify(initial_Paths) === JSON.stringify(lastInitialPathsRef.current);
        if (isAnnotationContentEqual && isPathContentEqual) {
            return;
        }

        if (!isAnnotationContentEqual) {
        ...
        lastInitialAnnotationsRef.current = initial_Annotations;
        lastInitialPathsRef.current = initial_Paths;

    }, [initial_Annotations, initial_Paths]);
